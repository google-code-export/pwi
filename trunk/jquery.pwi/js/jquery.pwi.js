	...
	$.fn.pwi = function(opts) {
		...
		function album(j) {
			...
			if ($np > settings.maxResults) {
				...
				if (settings.page > 1) {
					$ppage.addClass('link').bind('click.pwi',function(e){ e.stopPropagation(); settings.page = (parseInt(settings.page) - 1); getAlbum(); return false;});
				};
				$navRow.append($ppage);			
				...
				for (var i = 1; i < $pageCount + 1; i++) {
					if (i == settings.page) {
						...
						tmp = $("<div class='pwi_pager_page'>"+i+"</div>").bind('click.pwi',i, function(e){ e.stopPropagation(); settings.page = e.i; getAlbum(); return false });
					};
					$navRow.append(tmp);
				};
				
				if (settings.page < $pageCount) {
					$npage.addClass('link').bind('click.pwi',function(e){e.stopPropagation(); settings.page = (parseInt(settings.page) + 1); getAlbum(); return false });
				};
				$navRow.append($npage);
